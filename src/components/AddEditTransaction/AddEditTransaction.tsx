...
export function AddEditTransaction() {
  ...
  const { date, categoryId, value, note, id } = useSelector(
    (state: RootState) => state.addEditTransaction
  );
  ...
  function addOrEditTransaction() {
    if (transaction === undefined) {
      ...
      postTransaction(
        ...
        console.log("Transaction added on BE");
      });
      ...
      updateTransaction(id, parseFloat(value), date, categoryId, note).then(
        () => {
          console.log("Transaction updated on BE");
        }
      );
      dispatch(
        editTransaction({
  ...
  function transactionRemover() {
    if (transaction) {
      deleteTransaction(transaction.id).then(() => {
        console.log("Transaction deleted on BE");
      });
  ...
  function createTwoButtonAlert() {
    Alert.alert("Are you sure?", "This will delete the current transaction", [
      {
        text: "Cancel",
        onPress: () => {},
        style: "cancel",
